    ...
    <section className="bg-white py-8" id="prozess">
      <div className="section-shell">
        <div className="dark-panel rounded-2xl px-6 py-12 text-center text-white shadow-dark md:px-12">
          <h2 className="font-display text-3xl font-black tracking-normal md:text-4xl" data-reveal>
            IN <span className="text-gradient-gold">4 SCHRITTEN</span> ZUM ERSTEN BON
          </h2>
          <div className="mt-9 grid gap-7 lg:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr]">
            {steps.map(([num, title, text, Icon], index) => (
              <div key={num} className="contents">
                <article className="group relative rounded-t-full border border-gold/55 px-5 pb-8 pt-14 transition duration-300 hover:border-gold hover:bg-white/[0.03]" data-reveal>
                  <div className="absolute left-1/2 top-0 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-gradient-to-br from-gold-light to-gold text-lg font-black text-white shadow-premium transition duration-300 group-hover:scale-110 group-hover:shadow-glow">
                    {num}
                  </div>
                  <Icon className="mx-auto h-12 w-12 text-gold-light transition duration-300 group-hover:scale-110" />
