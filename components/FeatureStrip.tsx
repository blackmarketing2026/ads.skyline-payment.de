  ...
  return (
    <section className="-mt-1 bg-white py-8" id="funktionen">
      <div className="section-shell">
        <div className="dark-panel grid gap-6 rounded-2xl px-7 py-7 text-white shadow-dark md:grid-cols-3 lg:px-10">
          {items.map((item, index) => {
            const Icon = item.icon;
            return (
              <div key={item.title} className={`flex gap-5 ${index > 0 ? "md:border-l md:border-white/20 md:pl-8" : ""}`}>
                <Icon className="h-14 w-14 shrink-0 text-gold-light" />
                <div>
